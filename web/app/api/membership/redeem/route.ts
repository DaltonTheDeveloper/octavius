...
export async function POST(req: NextRequest) {
  ...
  const { extensionId, code } = (await req.json().catch(() => ({}))) as {
    extensionId?: string;
    code?: string;
  };
  if (!extensionId || !code) return NextResponse.json({ error: "extensionId + code required" }, { status: 400 });
  ...
  if (!ext.membershipCode || ext.membershipCode !== code) {
  ...
  const [license] = await db.$transaction([
    db.license.create({
      data: {
        userId: user.id,
        extensionId,
    ...
    db.redemption.create({ data: { userId: user.id, extensionId, code } }),
    db.extension.update({ where: { id: extensionId }, data: { downloads: { increment: 1 } } }),
  ]);
